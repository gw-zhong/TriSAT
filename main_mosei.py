...
from utils.dataset import load_mosei, load_mosei_bert
from utils.tools import Logger, seed_everything
from utils.train_mosei_transformer import train
...
def main(args):
    ...
    if args.use_bert:
        train_set, valid_set, test_set, input_dims = load_mosei_bert(data_dir)
    else:
        train_set, valid_set, test_set, input_dims = load_mosei(data_dir)
...
if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser(description="mosei")

    arg_parser.add_argument(
        '--run_id', dest='run_id', type=int, default=102)

    arg_parser.add_argument(
        '--num_epochs', dest='num_epochs', type=int, default=100)
    arg_parser.add_argument(
        '--patience', dest='patience', type=int, default=5)

    arg_parser.add_argument(
        '--signature', dest='signature', type=str, default='mosei')
    arg_parser.add_argument(
        '--cuda', dest='cuda', type=bool, default=True)
    arg_parser.add_argument(
        '--use_bert', dest='use_bert', type=bool, default=True)

    arg_parser.add_argument(
        '--data_dir', dest='data_dir', type=str, default='input')
    arg_parser.add_argument(
        '--model_dir', dest='model_dir', type=str, default='models/mosei')
    arg_parser.add_argument(
        '--output_dir', dest='output_dir', type=str, default='results/mosei')
    ...
    study = optuna.create_study(
        ...
        study_name='mosei[acc2]', storage=url, load_if_exists=True)
    ...
    df = study.trials_dataframe().to_csv(
        'results/mosei/[optuna][acc2]results_mosei_{}.csv'.format(args.run_id))
