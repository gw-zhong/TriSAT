...
from utils.multimodal_multihead_attention import MultiheadAttention
import math
...
class MultimodalTransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None:
                x, attn_weight = layer(x, x_k, x_v)
            else:
                x, attn_weight = layer(x)
            intermediates.append(attn_weight)
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        if x_k is None and x_v is None:
            x, attn_weight = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True)
            x, attn_weight = self.self_attn(query=x, key=x_k, value=x_v, attn_mask=mask)
