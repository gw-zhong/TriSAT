...
class LMF(nn.Module):
    ...
    def __init__(self, input_dims, hidden_dims, text_out, dropouts, output_dim, rank, use_softmax=False):
        ...
        self.audio_subnet = TextSubNet(self.audio_in, self.audio_hidden, self.audio_hidden, dropout=self.audio_prob)
        self.video_subnet = TextSubNet(self.video_in, self.video_hidden, self.video_hidden, dropout=self.video_prob)
    ...
    def forward(self, audio_x, video_x, text_x):
        ...
        text_x = self.text_model(text_x)  # BERT version
...
if __name__ == '__main__':
    t = torch.randn(128, 50, 300)
    v = torch.randn(128, 50, 20)
    a = torch.randn(128, 50, 5)

    model = LMF([5, 20, 300], [5, 20, 300], 300, (0.1, 0.1, 0.1, 0.1), 1, 1)
    out = model(a, v, t)
