...
class DynamicFusionGraph(nn.Module):
    ...
    def __init__(self, pattern_model, in_dimensions, out_dimension, efficacy_model):
        ...
        self.powerset = list(
            chain.from_iterable(combinations(range(self.num_modalities), r) for r in range(self.num_modalities + 1)))[
                        1:]
        ...
        self.efficacy_model = nn.Sequential(
            *[nn.Linear(sum(in_dimensions), list(efficacy_model.children())[0].in_features), efficacy_model,
              nn.Linear(list(efficacy_model.children())[-1].out_features, self.total_input_efficacies)])
    ...
    def fusion(self, in_modalities):
        ...
        for key in self.powerset[self.num_modalities:]:
            ...
            # print('\nself.networks[key].device = ', next(self.networks[key].parameters()).device)
            # print('this_input.device = ', this_input.device)
            self.networks[key].cuda()
...
if __name__ == "__main__":
    ...
    pattern_model = nn.Sequential(nn.Linear(100, 8))
    efficacy_model = nn.Sequential(nn.Linear(100, 8))
    fmodel = DynamicFusionGraph(pattern_model, [300, 20, 5], 8, efficacy_model)

    out = fmodel(modalities)
    print(out[0].shape)  # (batch, 1)
